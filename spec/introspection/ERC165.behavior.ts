  ...
  describe('::ERC165', function () {
    ...
    describe('#supportsInterface', function () {
      it('returns true for ERC165 interface', async function () {
        expect(await instance.callStatic.supportsInterface('0x01ffc9a7')).to.be
          .true;
      });

      it('returns false for unknown interface', async function () {
        expect(await instance.callStatic.supportsInterface('0x00000000')).to.be
          .false;
      });

      it('returns false for invalid interface', async function () {
        expect(await instance.callStatic.supportsInterface('0xffffffff')).to.be
          .false;
      });

      for (let interfaceId of interfaceIds) {
        it(`returns true for interface ${interfaceId}`, async function () {
          expect(await instance.callStatic.supportsInterface(interfaceId)).to.be
            .true;
        });
      }
    });
