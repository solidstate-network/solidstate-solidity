...
describe('NFTRoyalty', () => {
  ...
  describe('#_getRoyaltyBPS(uint256)', () => {
    it('returns royalty for single token', async () => {
      ...
      expect(await instance.$_getRoyaltyBPS.staticCall(tokenId)).to.eq(royalty);
    });
  });
  ...
  describe('#_setRoyaltyBPS(uint256,uint16)', () => {
    it('sets royalty for single token', async () => {
      ...
      expect(await instance.$_getRoyaltyBPS.staticCall(otherTokenId)).not.to.eq(
        royalty,
      );
    });
    ...
    describe('reverts if', () => {
      it('royalty exceeds 10000 BPS', async () => {
        ...
        ).to.be.revertedWithCustomError(instance, 'NFTRoyalty__RoyaltyTooHigh');
      });
    });
  });
