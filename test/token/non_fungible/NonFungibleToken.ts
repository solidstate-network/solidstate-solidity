...
describe('NonFungibleToken', () => {
  ...
  describe('#_isApprovedOrOwner(address,uint256)', () => {
    it('returns true if given spender is approved for given token', async () => {
      ...
      expect(
        await instance.$_isApprovedOrOwner.staticCall(spender.address, tokenId),
      ).to.be.true;
    });
    ...
    it('returns true if given spender is owner of given token', async () => {
      ...
      expect(
        await instance.$_isApprovedOrOwner.staticCall(holder.address, tokenId),
      ).to.be.true;
    });
    ...
    describe('reverts if', () => {
      it('token does not exist', async () => {
        ...
        await expect(
          instance.$_isApprovedOrOwner.staticCall(ethers.ZeroAddress, tokenId),
    ...
    describe('reverts if', () => {
      it('sender is not token owner', async () => {
        ...
        await expect(
          instance
            .connect(sender)
            .$_safeTransfer(ethers.ZeroAddress, sender.address, tokenId, '0x'),
        ).to.be.revertedWithCustomError(
          instance,
          'NonFungibleToken__NotTokenOwner',
        );
      ...
      it('receiver is the zero address', async () => {
        ...
        await expect(
          instance
            .connect(sender)
            .$_safeTransfer(sender.address, ethers.ZeroAddress, tokenId, '0x'),
        ).to.be.revertedWithCustomError(
          instance,
          'NonFungibleToken__TransferToZeroAddress',
        );
