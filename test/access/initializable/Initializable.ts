...
describe('Initializable', () => {
  ...
  describe('initializer() modifier', () => {
    it('sets initialized version to 1', async () => {
      ...
      expect(await instance.$_getInitializedVersion.staticCall()).to.equal(1n);
    });
  ...
  describe('reinitializer(uint8) modifier', () => {
    ...
    describe('reverts if', () => {
      it('version is less than or equal to previously initialized version', async () => {
        await expect(instance.$reinitializer(0n)).to.be.revertedWithCustomError(
          instance,
          'Initializable__AlreadyInitialized',
        );
        ...
        await expect(instance.$reinitializer(1n)).to.be.revertedWithCustomError(
          instance,
          'Initializable__AlreadyInitialized',
        );

        await expect(instance.$reinitializer(2n)).to.be.revertedWithCustomError(
          instance,
          'Initializable__AlreadyInitialized',
        );
      });
  ...
  describe('#_getInitializedVersion()', () => {
    it('returns initialized version', async () => {
      expect(await instance.$_getInitializedVersion.staticCall()).to.equal(0n);
