...
contract ERC20Extended is ERC20Base {
  ...
  function increaseAllowance (address spender, uint amount) virtual public returns (bool) {
    _approve(msg.sender, spender, _ds_ERC20Base().allowances[msg.sender][spender].add(amount));
    return true;
  }

  function decreaseAllowance (address spender, uint amount) virtual public returns (bool) {
    _approve(msg.sender, spender, _ds_ERC20Base().allowances[msg.sender][spender].sub(amount));
    return true;
  }
}
