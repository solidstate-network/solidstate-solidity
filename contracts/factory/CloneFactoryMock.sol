...
contract CloneFactoryMock is CloneFactory {
  ...
  function __deployClone (bytes32 salt) external returns (address cloneContract) {
    return _deployClone(salt);
  }

  function __calculateCloneDeploymentAddress (bytes32 salt) external view returns (address) {
